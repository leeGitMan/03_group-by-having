...
-- 부서별 평균 급여가 3백만원 이상인 부서를 조회하고 부서코드 오름차순


SELECT DEPT_CODE , ROUND(AVG(SALARY)) "나는 멍청하다"
FROM EMPLOYEE
--WHERE SALARY >= 3000000 -->> 한 사람의 급여가 3백만 이상이라는 조건
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE로 묶은 그룹 중 급여 평균이 3백만 이상인 그룹만 남은 거
ORDER BY DEPT_CODE;



-- EMPLOYEE 테이블에서 직급별 인원 수가 5명 이하인 직급코드, 인원 수 조회(직급 코드 오름차순)

SELECT JOB_CODE, COUNT(*) "후........"
FROM EMPLOYEE
HAVING COUNT(*) <= 5 -- HAVING 절에는 반드시 그룹 함수가 작성된다.
GROUP BY JOB_CODE 
